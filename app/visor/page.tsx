...
export default function Visor() {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (!token) {
          setStatus("❌ Falta token. Debes abrir el link con ?token=XXXX");
          setAutorizado(false);
          return;
        }

        // tabla: share_links (token, is_active, expires_at opcional)
        ...
        if (res.error) {
          setStatus("❌ Error verificando acceso: " + res.error.message);
          setAutorizado(false);
          return;
        }
  ...
  async function generarEstadoCuentaPDF() {
    try {
      ...
      const movimientosBody = movFiltrados.map((m) => {
        ...
        return [fecha, tipoTxt, String(m.description || ""), `$${total.toFixed(2)}`, `$${subtotal.toFixed(2)}`, `$${iva.toFixed(2)}`, `${pct}%`];
      });
      ...
      const facturasBody = facFiltradas.map((f) => {
        ...
        return [fecha, String(f.numero || ""), String(f.cliente || ""), `$${total.toFixed(2)}`, `$${subtotal.toFixed(2)}`, `$${iva.toFixed(2)}`, `${pct}%`, String(f.estado || "")];
      });
  ...
  return (
    <main style={page}>
      ...
      <p style={{ color: autorizado ? "#00ff88" : "#ff4d4d" }}>{status}</p>
      ...
      {!autorizado ? (
        <section style={panel}>
          ...
          <p style={{ color: "#aaa", marginTop: 10 }}>
            (Si tú quieres, te dejo un botón en el panel normal para crear/copiar ese link.)
          </p>
        </section>
      ) : (
        <>
