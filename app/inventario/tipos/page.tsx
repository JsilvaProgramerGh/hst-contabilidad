...
export default function TiposCamposPage() {
  ...
  async function createField() {
    ...
    const labelText = fLabel.trim();
    if (!labelText) return alert("Falta el nombre del campo (label).");
  ...
  async function saveEditField(id: string) {
    ...
    const labelText = String(e.label || "").trim();
    ...
    if (input_type === "select") {
      const raw: string[] = Array.isArray(e.options)
        ? (e.options as string[])
        : String(e.options ?? "")
            .split(",")
            .map((s) => s.trim())
    ...
    if (!labelText) return alert("Label vacío.");
  ...
  return (
    <main style={page}>
      <header style={header}>
        ...
        <a href="/inventario" style={linkBack}>
          ← Volver
        </a>
      </header>
      ...
      <div style={grid2}>
        ...
        <section style={card}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            />
            <button style={btnSmall} onClick={createType}>
              Crear
            </button>
          </div>
          ...
          <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
            {types.map((t) => (
              ...
              >
                <button style={rowPick} onClick={() => setSelectedTypeId(t.id)} title="Seleccionar">
                  {t.name}
                </button>

                <div style={{ display: "flex", gap: 8 }}>
                  <button style={softBtn} onClick={() => renameType(t.id)}>
                    Renombrar
                  </button>
                  <button style={dangerBtn} onClick={() => deleteType(t.id)}>
                    Eliminar
                  </button>
                </div>
              </div>
          ...
          {!selectedTypeId ? (
            ...
            <>
              ...
              <div style={{ ...box, marginBottom: 14 }}>
                <div style={grid4}>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Ej: talla (si lo dejas vacío, se genera)"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      style={input}
                      value={fInputType}
                      onChange={(e) => setFInputType(e.target.value as TypeField["input_type"])}
                    >
                ...
                <button style={btn} onClick={createField}>
                  Agregar campo
                </button>
              </div>
                    ...
                    return (
                      <div key={f.id} style={rowField}>
                          ...
                          {!isEdit ? (
                            <>
                              ...
                              <div style={{ color: "#aaa", fontSize: 12 }}>
                                input: <b>{f.input_type}</b> · requerido: <b>{String(f.required)}</b> · orden:{" "}
                                <b>{f.sort}</b>
                            ...
                            <>
                              <div style={grid4}>
                                ...
                                <div>
                                  ...
                                  <select
                                    style={input}
                                    value={(ed.input_type as TypeField["input_type"]) || "text"}
                                    onChange={(e) =>
                                      setEditing((p) => ({
                                        ...p,
                                        [f.id]: { ...p[f.id], input_type: e.target.value as TypeField["input_type"] },
                                      }))
                                    }
                                  >
                                    <option value="text">text</option>
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setEditing((p) => ({
                                        ...p,
                                        [f.id]: { ...p[f.id], sort: Number(e.target.value) },
                                      }))
                                    }
                                  />
                                </div>
                        ...
                        <div style={{ display: "flex", gap: 8, alignItems: "start" }}>
                          {!isEdit ? (
                            <>
                              <button style={softBtn} onClick={() => startEditField(f)}>
                                Editar
                              </button>
                              <button style={dangerBtn} onClick={() => deleteField(f.id)}>
                                Eliminar
                              </button>
                            </>
                          ) : (
                            <>
                              <button style={btnSmall} onClick={() => saveEditField(f.id)}>
                                Guardar
                              </button>
                              <button style={softBtn} onClick={() => cancelEditField(f.id)}>
                                Cancelar
                              </button>
                            </>
                          )}
...
const rowPick: React.CSSProperties = {
...
const grid2Inside: React.CSSProperties = {
  display: "grid",
  gap: 10,
